...
<article id="post-6" class="post-6 page type-page status-publish hentry">
	...
	<header class="entry-header"><!-- .TITLUL -->
		<h2 class="entry-title"><a href="/" rel="bookmark">Incarcare candidati</a></h2>	</header><!-- .entry-header -->
...
$sql ="SELECT ID_client,Nume,Prenume,Unitate,Sector FROM wp_useri";
$result = mysqli_query($conn,$sql);
while($row=mysqli_fetch_assoc($result))
{
  if($_SESSION['userindex']==$row['ID_client'])
  {
     $scoala= $row['Unitate'];
     $sector=$row['Sector'];
     $prof_nume = $row['Nume']." ".$row['Prenume'];
     break;
  }
}
$nume=$_POST['nume'];
...
$clasa=$_POST['clasa'];
$prof_indr=$_POST['prof_indr'];
$verificare=$_POST['verificare'];
$adaugatde=$_SESSION['userindex'];

if($nume!=""&&$prenume!=""&&$prof_indr!="")
{
    $sql ="INSERT INTO inscrieri (Nume,Prenume,Clasa,Profesor,Scoala,Sector,Adaugat_de) VALUES ('$nume','$prenume','$clasa','$prof_indr','$scoala','$sector','$adaugatde')";
    if (mysqli_query($conn, $sql)) 
    {
      echo "<script>window.location = '/inscriere.php';</script>";
    }
    else 
    {
...
<form action="informatica-incarcare-candidati.php" method="post">
<input type="hidden" name="verificare" value="true">
...
<p style="text-align: left;">Clasa:    <select name="clasa" type="integer" >             /*STIU CA ARATA CA NAIBA... NU STIU ALTA METODA ... DE CAUTAT SI SCHIMBAT.*/
...
<p style="text-align: left;">Profesor indrumator:<input name="prof_indr" type="text" value="';if($prof_indr==""){echo $prof_nume;}else{echo $prof_indr;}

echo '"/></p>
<p style="text-align: left;">Scoala:<input name="scoala" type="text" readonly="true" value="'.$scoala.'"/></p>
<p style="text-align: left;">Sector:<input name="sector" type="text" readonly="true" value="'.$sector.'"/></p>
</td>
</tr>
<tr>
<td style="text-align: center;" align="left"><input type="submit" value="Trimitere" name="Shit"/></td>
</tr>
</tbody>
</table>
</form>';
...
echo'Fie nu aveti un nivel destul de mare pentru a avea acces la aceasta pagina, fie nu sunteti logat.';
?>
	</div><!-- .entry-content -->
</article><!-- #post-## -->
		</main><!-- .site-main -->
