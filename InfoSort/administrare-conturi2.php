...
$nr_ids=0;
$sql ="SELECT ID_client FROM wp_useri";
$result = mysqli_query($conn,$sql);
while($row=mysqli_fetch_assoc($result))
{
$ids[$nr_ids]=$row['ID_client'];
$nr_ids = $nr_ids + 1;
}
...
$sql = "UPDATE wp_useri SET "; // ------------------ un i in plus
...
$sql = "$sql {$variabile[$nr]} = '{$_POST[$variabile[$nr]][$a]}' ";
// adding last one
$sql = "$sql WHERE ID_client={$ids[$a]}";
//echo "{$sql} <br />";  echo de test
mysqli_query($conn, $sql);
}//end for ($a)
echo "Operatiune reusita!"; 
}
?>
<a href="administrare-conturi.php">Administrare conturi</a>
	</div><!-- .entry-content -->
</article><!-- #post-## -->
		</main><!-- .site-main -->
	</div><!-- .content-area -->
