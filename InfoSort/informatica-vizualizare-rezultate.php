...
while($row=mysqli_fetch_assoc($result))
{
  if($row['Clasa']==$clasa||$clasa==" ")
  {
     ...
     {
            if($row['Adm_Resp']==2){$admresp='Absent';}else  if($row['Adm_Resp']==1){$admresp='Respins';}else if($row['Adm_Resp']==0){$admresp='Admis';}
            echo "<tr><td> {$row['Nume']} </td><td> {$row['Prenume']} </td><td> {$row['ID_Elev']} </td><td> {$row['Nota1']} </td>
<td> {$row['Nota2']} </td><td> {$row['Media']} </td>
<td> {$row['Premiul']} </td><td> $admresp </td></tr>";
     } 
  }
}
echo'</table>';
