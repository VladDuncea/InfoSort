...
$sql ="SELECT * FROM wp_useri";
$result = mysqli_query($conn,$sql);
$variabile = array("Nume","Prenume","User","Parola","Email","Sector","Unitate","Userlevel",);
echo '<form action="/administrare-conturi2.php" method="post" >'; // incepe from
echo '<table style="font-size:80%">';
echo "<tr><td><b> Nume </b></td><td><b> Prenume </b></td><td><b> User</b></td><td><b> Parola </b></td><td><b> Email</b></td><td><b> Sector </b></td><td><b> Unitate de invatamant </b></td><td> <b>Userlevel </b></td></tr>"; 
$nr=-1;
while($row=mysqli_fetch_assoc($result))
{
$nr=$nr+1;
echo '<tr>';
for ($a=0;$a<count($variabile);$a++)
{
echo "<td><input style=\"font-size:90%\" type=\"text\" name=\"{$variabile[$a]}[{$nr}]}\" value=\"{$row[$variabile[$a]]}\"></td>";
}
echo '</tr>'; 
}
echo'</table>';

echo"<input type='submit' value='Trimitere' /></from>";
}
else echo "Nu aveti nivelul necesar pentru a vedea aceasta pagina";
?>
	</div><!-- .entry-content -->
</article><!-- #post-## -->
		</main><!-- .site-main -->
