	...
	<div id="sidebar" class="sidebar" style="position: fixed;">
		<header id="masthead" class="site-header" role="banner">
			<div class="site-branding">
				<font size="3"><?php  echo "$nume $prenume "; if($userlevell==2){ echo"<a href=\"admin.php\">$userlevel</a><br>";} else{echo" $userlevel<br>";}
                                   if($userlevell==0)
{
...
<article id="post-6" class="post-6 page type-page status-publish hentry">
	...
	<header class="entry-header"><!-- .TITLUL -->
		<h2 class="entry-title"><a href="/" rel="bookmark">Update complet</a></h2>	</header><!-- .entry-header -->
...
$nr_ids=0;
$nr_ids=$_SESSION['numar_randuri'];
$variabile = array("Centru_examen",);
for ($a=0;$a<$nr_ids;$a++)
{
  $sql = "UPDATE {$_POST['page']} SET "; // ------------------ un i in plus
  //for ($nr=0;$nr<count($variabile)-1;$nr++)
 // {
   // $sql = "$sql {$variabile[$nr]} = '{$_POST[$variabile[$nr]][$a]}' , ";  
 // }//end for($nr)
  $nr=count($variabile)-1;
  $sql = "$sql {$variabile[$nr]} = '{$_POST[$variabile[$nr]][$a]}' "; // adding last one
  $sql = "$sql WHERE ID={$_POST['Index'][$a]}";
  //echo "{$sql} <br />"; // echo de test
  mysqli_query($conn, $sql);
}//end for ($a)
echo "Operatiune reusita!<br/>"; 
$_SESSION['numar_randuri']=0;
if($_POST['page']=="Elevi_Municipiu")
{//echo'<a href="informatica-vizualizare-candidati-municipiu.php">Inapoi</a>';
echo '<form method="post" action="informatica-vizualizare-candidati-municipiu.php">
<input type="submit" value="Inapoi"/>
<input type="hidden" value="da" name="search"/>';
echo"<input type='hidden' name='scoala_mem' value='{$_POST['scoala_mem']}'>
<input type='hidden' name='clasa_mem' value='{$_POST['clasa_mem']}'><input type='hidden' name='centru_mem' value=' '>";
echo '</form>' ;}   
else
{//echo'<a href="informatica-vizualizare-candidati.php">Inapoi</a>';
echo '<form method="post" action="informatica-vizualizare-candidati.php">
<input type="submit" value="Inapoi"/>
<input type="hidden" value="da" name="search"/>';
echo"<input type='hidden' name='scoala_mem' value='{$_POST['scoala_mem']}'>
<input type='hidden' name='clasa_mem' value='{$_POST['clasa_mem']}'><input type='hidden' name='centru_mem' value=' '>";
