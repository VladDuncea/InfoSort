...
<article id="post-6" class="post-6 page type-page status-publish hentry">
	...
	<header class="entry-header"><!-- .TITLUL -->
		<h2 class="entry-title"><a href="/" rel="bookmark">Rezultate</a></h2>	</header><!-- .entry-header -->
...
$sql ="SELECT * FROM  `inscrieri` 
ORDER BY  `inscrieri`.`Media` DESC ";
$result = mysqli_query($conn,$sql);
if($clasa)
...
<td> <input size ='3' name='Premiul[{$nr}]' value='{$row['Premiul']}'> </td><td> <select name='Adm_Resp[{$nr}]'>
            ...
            echo "<input type='hidden' name='Index[{$nr}]' value='{$row['ID']}'><input type='hidden' name='page' value='inscrieri'>";
            echo"<input type='hidden' name='scoala_mem' value='{$scoala}'><input type='hidden' name='clasa_mem' value='{$clasa}'></tr> ";
        $nr=$nr+1;
     } 
  }
}
$_SESSION['numar_randuri']=$nr;
echo'</table><input type="submit" value="Salveaza"></form>';
}
}
?>
	</div><!-- .entry-content -->
