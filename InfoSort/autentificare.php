...
<article id="post-6" class="post-6 page type-page status-publish hentry">
	...
	<header class="entry-header"><!-- .TITLUL -->
		<h2 class="entry-title"><a href="/" rel="bookmark">Autentificare</a></h2></header><!-- .entry-header -->
	<div class="entry-content"><!-- .CONTINUT-->
<?php
$writedUsername = $_POST['username'];
$writedPassword=$_POST['password'];
if($writedUsername)
{
$servername ="sql309.byethost18.com";
$username="b18_17476840";
...
//$writedUsername = $_POST['username'];
//$writedPassword=$_POST['password'];
$sql ="SELECT ID_Client,User,Parola,Userlevel FROM wp_useri";
$result = mysqli_query($conn,$sql);
$pass =" ";
while($row=mysqli_fetch_assoc($result))
{
  if($row['User']==$writedUsername)
  {
     $pass=$row['Parola'];
     $level=$row['Userlevel'];
     $userindex=$row['ID_Client'];
     break;
  }
}
if($pass==" ")
{
 ...
 // echo '<a href="autentificare"> Mai incearca</a>';
}
else
{
  if($pass==$writedPassword)
  {
    echo"<script>window.location = 'autentificare2.php';</script>";
...
<table style="height: 167px;" border="0" width="696px">
<tbody>
<tr>
<td><form method="post">
<p style="text-align: center;">Utilizator:<input name="username" type="text" />
Parola:<input name="password" type="password" /></p>

</form></td>
</tr>
<tr>
<td style="text-align: center;" align="center"><input type="submit" value="Trimitere" /></td>
</tr>
</tbody>
</table>
