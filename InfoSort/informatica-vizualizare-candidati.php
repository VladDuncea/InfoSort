...
<option value=" ">-</option>';
$sql ="SELECT Centru_examen FROM inscrieri";
$result = mysqli_query($conn,$sql);
$liceecentru = array();
$adresa=0;
while($row=mysqli_fetch_assoc($result))
...
$sql ="SELECT * 
FROM  `inscrieri` 
ORDER BY  `inscrieri`.`Nume` ASC ";
$result = mysqli_query($conn,$sql);
$numar=0;
if($clasa)
{
  echo '<form name="update" action="/update-candidati.php" method="post"><div style=\"overflow-x:auto;\"><table style="font-size:70%" >'; //-------------------------------- nu ai inchis
  echo "<div class=\"first-row\"><tr><td style=\"width:13%\"><b> Nume </b></td><td style=\"width:15%\"><b> Prenume </b></td><td style=\"width:8%\"><b> Sector </b></td><td><b> Profesorul indrumator </b></td><td><b> Scoala de provenienta </b></td><td><b> Centrul de examen </b></td></tr></div>";
  while($row=mysqli_fetch_assoc($result))
  {
 ...
 echo "{$row['Centru_examen']}";} 	 	
  ...
  echo "<input type='hidden' name='page' value='inscrieri'> ";
